            ...
            <SheetContent side="left" className="w-80">
                ...
                <div className="flex flex-col h-full">
                    <div className="space-y-1 px-2 pb-3 pt-6">
                        {NAVIGATION_CONFIG.main.map(item => renderNavLink(item, true))}
                        {session && (
                            <>
                                {NAVIGATION_CONFIG.user.map(item => renderNavLink(item, true))}
                                {session.user?.role === "SELLER" &&
                                    NAVIGATION_CONFIG.seller.map(item => renderNavLink(item, true))}
                            </>
                        )}
                    </div>
